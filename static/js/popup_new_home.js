...
document.querySelector('.apartment_listing_new_home').addEventListener('click', async function (event) {
    ...
    if (clickedElement) {
        ...
        const dataRegion = clickedElement.getAttribute('data-region');
        console.log(dataRegion);
        json_element_buildify = JSON.parse(localStorage.getItem('home_new'));
        await replacing_information_in_a_modal(json_element_buildify[dataRegion][dataIndex]);
        ...
        let modal_property_features = await features_parser(json_element_buildify[dataRegion][dataIndex]["objectID"]);
